...
- **REQ-02** The **skill owns intent routing inside the agent session**: it maps inspection intent to `agent-fork session --json` and fork intent to `agent-fork fork ... --require-agent --json`, parses machine output, and renders the returned result prominently. It delegates host/session detection and all Git mechanics to the CLI. No skill-side executable or Git implementation is permitted.
