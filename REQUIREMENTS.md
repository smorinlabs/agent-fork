...
| 1 | Runtime failure (materialize failed → **after** rollback per §4; unreadable repo; parent-untouched assert tripped) **Amended 2026-08-20 (owner, P02 A5):** also `entry_unreadable` (a carried entry could not be read and did not meet the skip preconditions) and `strict_skip_refused` (`--strict` refused a fork that would have skipped entries) |
...
- **REQ-21 Materialize (exact-copy mode):** the verbatim sequence — staged diff `--binary --cached` → `apply --index`; unstaged diff → `apply`; untracked via NUL-delimited `ls-files --others`; ignored via the second `--ignored` pass only when enabled. Symlinks verbatim, permission bits preserved, parent strictly read-only. **Amended 2026-08-20 (owner, P02 A5):** an untracked or ignored entry that cannot be carried — unreadable, or a non-regular type — is **skipped and named** rather than aborting the fork, but only when its `lstat` succeeded and the fork carries no deletion; otherwise the operation raises `entry_unreadable`. Tracked paths are never skipped. `--strict` converts every skip into `strict_skip_refused`. **Amended 2026-08-10 (owner, Apple Git portability correction to test-architecture spec A3):** Intent-to-add entries are supported: cached diff uses `--ita-invisible-in-index`; each ITA working-tree patch is transported via plain `git apply`, then its child path is marked with `git add --intent-to-add -- <path>`; verification is ITA-aware. Production must not use `git apply --intent-to-add`, which can replace unrelated index entries under Apple Git 2.50.1.
