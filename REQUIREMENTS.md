...
- **REQ-31** `agent-fork cleanup <TARGET>` (fork name, branch, or worktree path): removes the worktree (`git worktree remove` + prune), deletes the fork branch **unless** `--keep-branch`, updates the fork registry. Never touches the parent checkout; refuses targets it didn't create unless `--force` (registry-based ownership check — D12). **Amended 2026-08-17 (P02-A3):** ownership is established by observation, never by a registry record alone. A record names a candidate; the invoking repository's live worktree list decides. A candidate whose worktree and branch are not a live pair there is refused with `cleanup_registry_stale`, and `--force` does **not** override that refusal — it remains the only evidence that the worktree still belongs to the repository its record names. Several live candidates for one target refuse with `cleanup_registry_ambiguous`. Both refusals are raised before any Git mutation. Consequently a fork name resolves against the invoking repository, so the same name may exist in several repositories.
- **REQ-31a** `agent-fork prune` (added 2026-08-17, P02-A3; absorbs the `prune` half of fault A7): removes fork-registry records whose recorded worktree path does not exist, and — **amended 2026-08-20 (gate-6 round 3)** — records carrying no repository, which can never authorize anything and can never gain an identity; clearing one destroys no work, because a worktree still at that path stays on disk and remains removable by explicit path. It changes only the registry — no worktree, branch, or other Git state is touched — so it is safe to run from anywhere, and unlike cleanup it needs no repository context, because absence is repository-independent. A record whose path exists but no longer holds that fork is reported and kept: the path may since have been taken by another repository's live worktree, and historical path equality is not proof of ownership. Requires confirmation unless `--yes`; `--dry-run` previews.
- **REQ-32 Guards (exit 5 unless `--force`):** uncommitted changes in the fork worktree · commits not on any upstream (unpushed) · target is the current working directory. **Amended 2026-08-09 (owner, pre-merge review):** `--force` overrides the dirty/unpushed guards only (D12; the invoking-cwd refusal is not overridable). It does **not** replace consent (R8.1). **Amended 2026-08-20 (P02 A10):** the default retention of a cleaned-up worktree's planned lineage claim, inferred parent record, and freshness corroboration does not change — they preserve resumability and remain Agent Fork's strongest local parent evidence. Both real and `--dry-run` cleanup now disclose exactly what is retained: an additive `retained_metadata` object (`lineage_claims`, `inferred_records`, `freshness_entries`, one source-qualified `removal_commands` entry per retained record) in machine output, and matching notices in human output. No new flag prunes this metadata; the owner explicitly declined an opt-in pruning flag in favor of disclosure plus the existing `session claude-parent delete` command.
...
- **REQ-41a** Registry schema (added 2026-08-17, P02-A3): the on-disk registry is version 2, and each record carries the resolved Git common directory of the repository that owns it. Fork-time replacement is keyed on `(repository, name)`, so two repositories may each hold a fork of the same name. Version 1 records are read and preserved with no repository, never rejected and never inferred — the only path such a record holds is where its worktree was when it was written. **Amended 2026-08-20 (owner decision, gate-6 round 3):** such a record authorizes nothing and is never backfilled. Matching a live worktree does not establish ownership, because two repositories can hold one path on one branch name — ordinary under the `central` worktree layout, which keys on a repository's basename alone. `prune` clears these records; the forks themselves remain removable by explicit path. Registry removal is a compare-and-swap on the selected record. Once version 2 is written, earlier releases will not read the file.
