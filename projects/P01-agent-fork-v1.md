...
- [x] [P01-TS26] Lock D20/REQ-49, repair the related corpus and conformance drift, extend T-SES-22, and add T-SES-23..27 as behavior-RED tests with matrix tracking intact — RED observed before implementation; rows green
- [x] [P01-T39] Extend the existing `session` result with additive invocation-directory, repository-topology, default-branch, worktree, and Git-status context across every identity outcome — G-SES T-SES-22..27 green
- [x] [P01-T40] Replace `scripts/fork_session.py` with direct agentic-skill routing for session inspection, explicit names, and CLI-owned automatic topic-branch naming while preserving diagnostic and output contracts — wrapper deleted; 8 focused tests and skill validation green
- [x] [P01-T41] Close README, corpus, conformance, skill metadata, placement, full local gates, and fresh Claude Code/Codex forward tests; stop before release or publish work — dual-host dev placements and all approved gates green; no release work started

Read-only native session-fork command follow-up (gate: reconciled dual-agent review, TDD, dual-host acceptance; stop before release)
- [x] [P01-TS27] Lock D21/REQ-50, reconcile the dual-agent adversarial findings, allocate T-EMT-08..10 and T-SES-28..32, and observe behavior-RED tests — 13 intended RED failures observed; matrix stayed green
- [x] [P01-T42] Implement shared native rendering, safe construction, status invariants, and one-UUID-per-inspection serialization — focused and full suites green
- [x] [P01-T43] Wire JSON, human output, help examples, and exact `--session`/`--session-only` skill routing — skill validation and static route gates green
...
- [x] [P01-TS29] Add RED coverage for the `--now` token, candidate-name resolution, the dry-run confirmation, and the `--now` fast path — RED observed before each of four implementation steps (369 → 373); 373 passed/1 skipped/9 deselected
...
- [x] [P01-T52] Add per-agent output presentation references to the companion skill — `references/output-claude.md` and `references/output-codex.md` encode the agreed formats: table-based summaries for `--session`, the fork confirmation plan, and the fork result, plus a bare one-line `--session-only` response; SKILL.md gains a formatting-only loader keyed on the reported `agent`; no CLI or route changes; locked by tests/skill/test_output_references.py
- [ ] [P01-T51] Pin the branch-derived candidate between the confirmation dry run and the real run — `_fork_cli` recomputes `derive_auto_name`/`unique_auto_name` whenever the positional name is omitted, so a branch or destination collision appearing between the two calls can give the real fork a different suffix than the user approved; the robust fix is a stable plan token in the CLI's dry-run output, which is a CLI change rather than a skill change (deferred from the PR #23 review)
