...
- [x] [P01-T52] Add per-agent output presentation references to the companion skill — `references/output-claude.md` and `references/output-codex.md` encode the agreed table-based formats for `--session`, `--session-only`, the fork confirmation plan, and the fork result; SKILL.md gains a formatting-only loader keyed on the reported `agent`; no CLI or route changes; locked by tests/skill/test_output_references.py
