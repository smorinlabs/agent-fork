...
- [x] [P02-T06] A6 remediation umbrella — complete after **A6a** (`--ignore-submodules` filtering plus an accurate notice) and **A6b** (carry submodules identically by default, `--with-submodules` / `--no-with-submodules`, owner decision 2026-08-17) both merged to `main`; A6b merged via PR #67 on 2026-08-22. The work split on 2026-08-20 after two gate-4 passes returned NOT-READY on integration complexity, not on the carry mechanism. Gate 6 (adversarial implementation review) completed four rounds; subsequent independent production review and PR feedback were absorbed with evidence rows through T-OUT-35. Final pre-merge gate: `just all` 760 passed/1 skipped/20 deselected, the 701-row test matrix was clean, and `just test-git-matrix` was green on Git 2.50.1 and 2.54.0. Plan and all gate reports are in the [design doc](../docs/superpowers/plans/2026-08-17-p02-a6-dirty-submodules.md).
