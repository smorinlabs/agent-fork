...
- [x] [P02-T10] A10 fix per process — inferred-parent freshness now preserves last-known-good evidence without accepting it as strict proof, the freshness index and transcript cache are bounded and safely migrated, corpus limits are typed exit-3 refusals, and cleanup discloses retained metadata. Gate 6 completed after two independent review rounds; merged in [PR #63](https://github.com/smorinlabs/agent-fork/pull/63). Final merged gates: 583 passed, 1 expected skip, 9 deselections; matrix, strict-collection, and clean-install checks green. See the [A10 design and evidence record](../docs/superpowers/plans/2026-08-18-p02-a10-claude-freshness.md).
