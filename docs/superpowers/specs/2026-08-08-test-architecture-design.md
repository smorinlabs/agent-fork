...
| Axis | Values |
|---|---|
| `mode` | three resolved-plan tokens: **`exact`** (state ✓, ignored ✗ — the baseline) · **`exact+ignored`** (✓ ✓) · **`no-state`** (✗ ✗, clean-from-HEAD) — always reached via the real toggles (`--no-with-state`, `--with-ignored`, config keys); no `--clean` exists (D2). The three config-vs-flag conflict scenarios are **G-CFG rows**, not axis values, with corpus-pinned outcomes (RESEARCH §1.1 implication rule — it wins even over an explicit false): config `with_state=false` + `--with-ignored` → `exact+ignored`; config `with_ignored=true` + `--no-with-state` → `exact+ignored` (asserted + documented as a UX note); all-unset → `exact`. |
...
**Row statuses** (machine-readable `row_status` column in every table): `live` · `n/a` · `tombstone` (removed — never returns; no stub may exist; e.g. the pre-0.95 detection ladder, A7) · `retired` (returns at a named milestone; keeps an exempt skip stub; e.g. E4, A8) · `blocked` (awaiting a ruling). **Two blocked classes exist at spec time**, each with a named unblock gate: G-REG lock-wait assertion rows (gate: the REQUIREMENTS lock-semantics amendment, §5) and below-`PRODUCT_GIT_MIN` doctor rows (gate: A9's REQUIREMENTS amendment).
...
- **G-MAT** includes: ITA transport rows (A3 — both platforms verified: naive transport leaves the child `??` vs parent ` A`, so the ladder catches it; the fix uses `--ita-invisible-in-index` + `apply --intent-to-add`), empty-dir contract row (documented absence — "git-visible state copy"), nested-untracked-dir rows, submodule rows (gitlink OIDs compared, contents pruned from manifest).
...
- **G-REG** includes: the different-name concurrent registry-write race (the true REQ-41 atomicity row). Lock-wait/timeout semantics are currently undefined in the corpus — like `PRODUCT_GIT_MIN` (A9), they must land as a REQUIREMENTS amendment during implementation; the race row's assertion cells carry `row_status: blocked` with that amendment as the unblock gate.
...
A twelfth pending amendment is flagged, not yet decided: **registry lock-wait/timeout semantics** (REQ-41/R5.8 gap; the G-REG blocked rows' unblock gate). Executing A1–A11 into the corpus — including updating IMPLEMENTATION-PROMPT §1's precedence list — is **step 0 of §10**; until then this spec supersedes the corpus per §1.
...
0. **Execute §8's amendments into the corpus** — REQUIREMENTS.md, DESIGN-DECISIONS.md, and IMPLEMENTATION-PROMPT §1's precedence list — restoring corpus precedence (§1).
