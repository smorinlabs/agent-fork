...
- Total rows: 543 (20 groups; recount whenever a group's table changes — see spec §4's ~120–150 estimate, superseded by approved per-group density).
...
| T-INC-21 | A12 gate-6 round 4 — a hook group whose leader is still running is signalled without a `killpg(pgid, 0)` probe first: a session leader cannot leave its own group and an unreaped child pins its pid, so the answer is already known and the saved syscall is one fewer step between the last confirmation and the signal (Known limit 9 covers the reaped-leader remainder) | baseline | F | live | P02 A12 gate-6 round 4; REQ-24 |
