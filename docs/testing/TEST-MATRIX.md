...
- Total rows: 219 (18 groups; recount whenever a group's table changes — see spec §4's ~120–150 estimate, superseded by approved per-group density).
...
| T-VER-12 | A1 negative (a) — ambient `apply.whitespace=fix` strips trailing whitespace from an unstaged hunk during transport; porcelain unchanged, raw bytes diverge; verify must fail `content-match` and roll back | baseline | F | live | A1 design doc §Design item 1; gate-1 repro |
