...
| T-MAT-12 | intent-to-add file transported → cached diff uses `--ita-invisible-in-index`, working-tree patch uses plain `apply`, child is marked via `add --intent-to-add`, child shows ` A` not `??`, and existing index entries remain intact | baseline | F | live | REQ-21 (A3) |
