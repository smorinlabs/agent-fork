...
| T-GRD-30 | PR #67 Greptile review — an initialized gitlink with no matching `.gitmodules` path entry refuses as `submodule_unrepresentable` before branch/worktree mutation; dry-run and real fork agree | baseline | F | live | PR #67 review |
| T-GRD-31 | Production finding 1 — a cold submodule directory containing an ordinary file refuses as `submodule_cold_content` before branch, destination, registry, or parent-state mutation | baseline | F | live | post-merge production review |
| T-GRD-32 | Production finding 1 — cold-content inspection reaches nested submodules and reports the qualified submodule and entry paths | baseline | F | live | post-merge production review |
| T-GRD-33 | Production finding 2 — an effective `url.*.insteadOf` prefix matching the pinned local checkout source refuses as `submodule_transport_unsafe`, with zero network requests and zero fork mutation | baseline | F | live | post-merge production review |
| T-GRD-34 | Production finding 4 — direct index stages 1–3 inside an initialized submodule refuse a real fork as `unmerged_index` before mutation even when the top-level index is merged | baseline | F | live | post-merge production review |
| T-GRD-35 | Production finding 4 — dry-run applies the same recursive unmerged-index guard and qualified path reporting as a real fork | baseline | F | live | post-merge production review |
| T-GRD-36 | Production finding 4 — recursive unmerged-index inspection reaches depth two and remains scoped to `with_state and with_submodules`, preserving both opt-outs | baseline | F | live | post-merge production review |
...
| T-MAT-31 | A5 — `--strict` aggregates two unreadable capture-phase entries into one `strict_skip_refused` error, names both in byte-wise path order, and rolls the worktree back | baseline | F | live | P02 A5 design; R6.3 |
| T-MAT-32 | A5 — an injected non-regular entry at the transport seam is skipped with an `unsupported-type` record and one notice rather than raising a raw materialization error | baseline | F | live | P02 A5 design; REQ-21 |
...
| T-MAT-36 | A5 — a staged `git rm --cached` deletion blocks the unreadable untracked working-file replacement even though no path is absent; `deletion_blockers[]` names it | baseline | F | live | P02 A5 design; gate-1 pass 5 |
| T-MAT-37 | A5 — `--strict --with-ignored` aggregates distinct capture-, materialize-, and `.worktreeinclude`-phase skips into one byte-wise ordered refusal | mode=exact+ignored | F | live | P02 A5 design; R6.3 |
| T-MAT-38 | A5 — under `--no-verify`, where capture is bypassed, an unreadable entry first encountered by materialize is skipped and named rather than failing the fork | mode=exact, verify=off | F | live | P02 A5 design; REQ-21 |
| T-MAT-39 | A5 PR review — under `--no-verify`, a regular source replaced by a symlink after `lstat` is never followed or copied; the materialize skip reaches the final sentinel and rolls the fork back | mode=exact, verify=off | F | live | P02 A5 PR review; REQ-21; REQ-23 |
| T-MAT-40 | A6b step 4 — no gitlinks snapshots to an empty tuple | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-41 | A6b step 4 — a clean submodule's snapshot records its own HEAD and resolved remote URL | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-42 | A6b step 4 — cell `g`, an uninitialized submodule snapshots as cold with no HEAD, URL, or nested plan | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-43 | A6b step 4 — cell `j`, a renamed submodule's snapshot keeps both config name and path | baseline | F | live | A6 design doc §Recipe step 0 |
| T-MAT-44 | A6b step 4 — cell `h`, a nested submodule produces one nested plan entry | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-45 | A6b step 4 — the snapshot captures a dirty submodule's own inventory and content state, not just its HEAD | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-46 | A6b step 4 — `with_state=False` snapshots to an empty tuple; nothing to carry, nothing to snapshot | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-47 | A6b step 4 — a relative `.gitmodules` URL resolves to an absolute value in the snapshot, matching what recipe step 3 needs "before the fork" | baseline | F | live | A6 design doc, gate-4 pass 4 finding 4 |
| T-MAT-48 | A6b step 4 — an unreachable remote URL is recorded from local config without any network access | baseline | F | live | A6 design doc §Recursive snapshot |
| T-MAT-49 | A6b step 5 — no submodules to carry when none exist | baseline | F | live | A6 design doc §Recipe |
| T-MAT-50 | A6b step 5 — cell `a`, a modified submodule initializes offline and both top-level and inner status match the parent exactly | baseline | F | live | A6 design doc §Recipe steps 1-2 |
| T-MAT-51 | A6b step 5 — cell `c`, an unstaged gitlink advance is represented by checking the child submodule out at the parent's submodule HEAD, not the index gitlink OID | baseline | F | live | A6 design doc §Recipe step 4 |
| T-MAT-52 | A6b step 5 — carrying never runs `git submodule sync` in the child; a deliberate parent config override survives | baseline | F | live | A6 design doc, gate-4 pass 1 finding 1 |
| T-MAT-53 | A6b step 5 — only the child's own `remote.origin.url` is restored, matching the parent's resolved value | baseline | F | live | A6 design doc §Recipe step 3 |
| T-MAT-54 | A6b step 5 — cell `j`, a renamed submodule initializes offline via the name-keyed URL override | baseline | F | live | A6 design doc §Recipe step 0 |
| T-MAT-55 | A6b step 5 — the offline override engages even against a genuinely unreachable `.gitmodules` remote URL | baseline | F | live | A6 design doc §Recipe step 2 |
| T-MAT-56 | A6b step 5 — cell `g`, a parent-cold submodule stays cold in the child; nothing is initialized | baseline | F | live | A6 design doc §Recipe step 1 |
| T-MAT-57 | A6b step 5 — `submodule.<name>.update=none` cannot make init silently no-op, because `--checkout` is always passed | baseline | F | live | A6 design doc, gate-4 pass 1 finding 2 |
| T-MAT-58 | A6b step 5 — cell `h`, a nested submodule is carried recursively rather than left cold | baseline | F | live | A6 design doc §Recipe step 6 |
| T-MAT-59 | A6b step 5 — cell `i`, a change staged in the submodule's own index (HEAD unmoved) transports via the reused `materialize` seam | baseline | F | live | A6 design doc §Recipe step 5 |
| T-MAT-60 | A6b step 5 — the carry notice states the configuration-fidelity limit: only `remote.origin.url` is restored | baseline | F | live | A6 design doc §Recipe step 3 |
| T-MAT-61 | A6b coverage gap — cell `b` (submodule dirtied only by untracked content) carries correctly through the recipe, not merely through top-level verification | baseline | F | live | A6 design doc §Matrix 1 cell `b`; coverage audit |
| T-MAT-62 | A6b coverage gap — cell `f` (a dirty submodule alongside an ordinary dirty file) carries through the recipe; the two transport mechanisms genuinely coexist | baseline | F | live | A6 design doc §Matrix 1 cell `f`; coverage audit |
| T-MAT-63 | A6b coverage gap — the offline URL override engages for a relative `.gitmodules` URL at the carry-recipe level, not only at the snapshot level | baseline | F | live | coverage audit |
| T-MAT-64 | A6b coverage gap — depth-2 dirt: a dirty tracked file inside a submodule nested two levels deep carries, distinct from cell `h`'s clean-nested case | baseline | F | live | A6 design doc §Implementation plan step 1; coverage audit |
| T-MAT-66 | Gate-6 finding 5 — a submodule config name containing a space parses correctly (`--null`, not a single-space split) and the offline override actually engages, proven against a remote-unreachable URL | baseline | F | live | gate-6 finding 5 |
| T-MAT-67 | Gate-6 finding 5 — a submodule config name containing `=` cannot be expressed as a `-c key=value` override; skipped with a notice rather than silently falling through to the real, uncontrolled remote | baseline | F | live | gate-6 finding 5 |
| T-MAT-68 | Gate-6 round 2 finding 7 — `submodule update --init` persists a synthetic `remote.origin.url` pointed at the parent's local checkout path; when the parent's own submodule has none, carry must remove it rather than let it survive | baseline | F | live | gate-6 round 2 finding 7 |
| T-MAT-69 | Gate-6 round 2 finding 8 — a nested submodule's notice text is qualified with its outer path (`outer/inner`), matching the qualification `carried`/`skipped` already receive, so two same-named nested submodules under different outers stay attributable | baseline | F | live | gate-6 round 2 finding 8 |
| T-MAT-70 | Gate-6 round 2 finding 10 — round-1 finding 6's terminal-escaping fix (commit bf81d94) had no regression test; a hostile submodule config name (ANSI erase-screen sequence) must come back escaped in the "submodule carried" notice | baseline | F | live | gate-6 round 2 finding 10; gate-6 finding 6 |
| T-MAT-71 | Production finding 2 — the carry primitive pins all network protocols off at mutation time, so bypassing preflight still makes a matching HTTP rewrite fail before a request despite ambient `protocol.http.allow=always` | baseline | F | live | post-merge production review |
| T-MAT-72 | Production finding 3 — every `remote.origin.url` value is restored in original order, not collapsed to one scalar | baseline | F | live | post-merge production review |
| T-MAT-73 | Production finding 3 — legal trailing spaces in `remote.origin.url` survive snapshot and carry byte-for-byte | baseline | F | live | post-merge production review |
| T-MAT-74 | CodeRabbit follow-up — a capture-time skip inside a carried submodule reaches the public result and notice with its top-level-qualified path | baseline | F | live | PR #67 review |
| T-MAT-75 | CodeRabbit follow-up — a skip first discovered during recursive materialization reaches the public result and notice with its top-level-qualified path | baseline | F | live | PR #67 review |
| T-MAT-76 | CodeRabbit follow-up — `--strict` refuses a recursive capture skip with exact qualified details and rolls the fork back | baseline | F | live | PR #67 review |
...
| T-VER-40 | A5 — excluding a known skipped nested path from `exact-copy-status` remains narrow enough that a missing non-skipped sibling is still detected | baseline | F | live | P02 A5 design; REQ-23 |
| T-VER-41 | A5 — verification re-capture consumes the known skip set and never reopens an already skipped path | baseline | F | live | P02 A5 design; REQ-23 |
| T-VER-42 | A5 — a setup hook that mutates a skipped parent entry after the verification ladder is caught by the finalization sentinel, fails with `verify_failed`, and rolls back before registry publication | baseline | F | live | P02 A5 design; REQ-23 |
| T-VER-43 | A5 — a skipped filename containing pathspec metacharacters is excluded literally and cannot hide a missing sibling that its characters would otherwise match | baseline | F | live | P02 A5 design; REQ-23 |
| T-VER-44 | A5 gate-6 — the sentinel is frozen from the same successful `lstat` that precedes the failed read, so an intervening target swap cannot become the accepted baseline | baseline | F | live | P02 A5 gate-6; REQ-23 |
| T-VER-45 | A5 gate-6 — a same-size in-place rewrite with inode, mode, size, and mtime restored still fails finalization because `st_ctime_ns` changed | baseline | F | live | P02 A5 gate-6; REQ-23 |
| T-VER-46 | A6b step 6 headline case — default settings, no flags, a dirty submodule: the fork succeeds and both statuses match exactly, top level and inside the submodule | baseline | F | live | A6 design doc; gate-4 pass 4 |
| T-VER-47 | A6b step 6 — `--no-with-submodules` still forks successfully, reproducing A6a's original opt-out behaviour, gated rather than deleted | baseline | F | live | A6 design doc §Split |
| T-VER-48 | A6b step 6 — recursive verification (HEAD-identity rung) catches a carried submodule detached at the wrong commit, even when every top-level signal agrees | baseline | F | live | gate-4 pass 3 finding 2 |
| T-VER-49 | A6b coverage gap — a mixed-time race between snapshot and carry is caught by verification: carry transports live bytes, verification compares against the frozen snapshot, so the race surfaces as a divergence rather than passing silently | baseline | F | live | coverage audit |
| T-VER-50 | A6b coverage gap — gate-4 pass 1 finding 4: ambient config threaded into a recursive `collect_inventory` call must not hide a nested submodule's advance. Gate-6 round 2 findings 4+5 strengthened the guarantee from a `-c` pin to an unconditional `--ignore-submodules=none` flag on `with_submodules=True`; this test now proves that stronger, caller-independent form rather than the pin explicitly | baseline | F | live | gate-4 pass 1 finding 4; coverage audit; gate-6 round 2 findings 4, 5; gate-6 round 3 |
| T-VER-51 | A6b coverage gap — `--with-ignored` inside a submodule: an ignored file carries and recursive verification's content rung must thread `with_ignored` rather than hardcoding `False`, which otherwise reports a correctly-carried ignored file as "no longer carried" | baseline | F | live | A6 design doc §Implementation plan step 1; coverage audit; found by this test (real bug, fixed) |
| T-VER-52 | Gate-6 finding 2 — the real snapshot→carry→verify pipeline, not the primitive directly: ambient `diff.ignoreSubmodules=all` at snapshot time must not cause a false "newly carried" difference when carry and verify, both pinned, correctly see what the unpinned snapshot missed | baseline | F | live | gate-6 finding 2 |
| T-VER-53 | Gate-6 finding 1 — rung 7 ("recursive parent-untouched") compared only dirty-inventory content, never the parent submodule's own HEAD; a clean commit-to-commit move between snapshot and verify must still be caught | baseline | F | live | gate-6 finding 1 |
| T-VER-54 | Gate-6 finding 7 — rung 6 compared the bare, unqualified `plan.path` against `skipped`, which `carry_submodules` qualifies with its outer prefix (`outer/inner`); a skipped NESTED plan could never be detected as a result | baseline | F | live | gate-6 finding 7 |
| T-VER-55 | Gate-6 round 2 findings 4+5 — a `submodule.<name>.ignore` local config value hides a staged gitlink advance from the inventory and from `materialize`'s patch generation; the `-c diff.ignoreSubmodules=none` pin cannot defeat it, only the explicit `--ignore-submodules=none` flag can, confirmed empirically | baseline | F | live | gate-6 round 2 findings 4, 5 |
| T-VER-56 | Gate-6 round 2 finding 1 — a `=`-named submodule's reasoned skip must not fail rung 6 ("nested-plan completeness") the way a silent/unexpected skip does; the fork still succeeds, with a notice, and the submodule stays cold | baseline | F | live | gate-6 round 2 finding 1 |
| T-VER-57 | PR #67 Copilot/CodeRabbit review — when recursive submodule verification is the only failing rung, `submodule-content-match` is the one structured primary check | baseline | F | live | PR #67 review |
| T-VER-58 | Production finding 3 — a post-carry fault injection corrupting the child's ordered `remote.origin.url` tuple fails recursive verification as `submodule-remote-url` and rolls back the branch and worktree | baseline | F | live | post-merge production review |
...
| T-RBK-12 | SIGTERM during `cleanup`'s `git worktree remove --force` → active Git process group killed, exit 143 with partial-removal warning and `agent-fork prune` remedy, registry record retained | baseline | F | live | REQ-31; cleanup signal window |
| T-RBK-13 | SIGTERM in `run_git` after `Popen` starts Git but before `_ACTIVE.process` registration → signal stays blocked until registration, then the handler kills the Git process group and exits 143 | baseline | F | live | REQ-22; PR #66 review |
...
| T-INC-22 | A5 — an unreadable `.worktreeinclude` match is skipped with exactly one notice, the fork succeeds, and the successful result records the include-phase skip | baseline | F | live | P02 A5 design; REQ-24 |
| T-INC-23 | A5 — under `--strict`, an unreadable `.worktreeinclude` match joins the aggregate `strict_skip_refused` error before registry publication and the fork rolls back | baseline | F | live | P02 A5 design; REQ-24 |
| T-INC-24 | A5 gate-6 — under `--with-ignored`, an unreadable ignored path already skipped during capture is not reopened, recorded again, or announced twice merely because it also matches `.worktreeinclude` | mode=exact+ignored | F | live | P02 A5 gate-6; A13; REQ-24 |
...
| T-OUT-29 | A5 — `strict_skip_refused` is cataloged at exit 1 and renders the exact byte-wise ordered `skipped[]` plus `count` JSON details schema | baseline | C | live | P02 A5 design; R7.2; R7.8; R7.12 |
| T-OUT-30 | A5 — `entry_unreadable` renders the exact escaped entry plus byte-wise ordered `deletion_blockers[]` JSON details schema | baseline | C | live | P02 A5 design; R7.2; R7.8 |
| T-OUT-31 | A5 — a successful skip notice is absent from stdout, appears on stderr exactly once, remains exactly once in JSON `notices[]`, and the JSON result retains the `skipped[]` entry | baseline | C | live | P02 A5 design; A13; R7.1; R7.2 |
| T-OUT-32 | A6b step 6 — `-o json` reports the resolved `with_submodules` flag under `fork.mode` and the carry outcome in `notices[]` | baseline | F | live | A6 design doc |
| T-OUT-33 | Production finding 1 — `submodule_cold_content` renders the qualified submodule path, bounded ordered entry list, and total count in exact JSON details | baseline | C | live | post-merge production review |
| T-OUT-34 | Production finding 2 — `submodule_transport_unsafe` renders the qualified submodule, local source, and matching rewrite prefix without disclosing the rewrite target URL | baseline | C | live | post-merge production review |
| T-OUT-35 | CodeRabbit follow-up — a successful recursive skip uses one top-level-qualified path in JSON `skipped[]`, JSON `notices[]`, and stderr | baseline | C | live | PR #67 review |
...
| T-CLI-68 | A5 — `fork --strict` is a false-default CLI-only boolean that parses and appears in help; no configuration or environment precedence surface is introduced | baseline | C | live | P02 A5 design; R3.3; R3.6 |
| T-CLI-69 | Gate-6 finding 3 — dry-run's own unstaged count hardcoded `--ignore-submodules=dirty` unconditionally, undercounting a submodule under the tool's own `with_submodules` default | baseline | F | live | gate-6 finding 3 |
| T-CLI-70 | Gate-6 round 2 finding 4 (second half) — the dry-run preview's own staged/unstaged count args need the same explicit `--ignore-submodules=none` flag `collect_inventory`/`materialize` gained, or a `submodule.<name>.ignore` local config makes the preview undercount what the real fork correctly carries | baseline | F | live | gate-6 round 2 finding 4 |
...
| T-FIX-35 | PR #67 CodeRabbit review — rewriting a fixture submodule URL commits only `.gitmodules`, preserving earlier staged state and leaving the gitlink for `_dirty_submodule`'s scoped commit | baseline | F | live | PR #67 review |
