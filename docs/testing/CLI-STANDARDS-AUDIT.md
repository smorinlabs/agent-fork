...
| Arguments/options (R3, R4) | conforming | kebab-case long flags, reserved short flags, no prefix abbreviation, standard global flags, output/JSON surface, boolean negations; G-CLI/G-CFG |
...
| Output/help/errors (R7) | conforming after CLI-AUD-01 | stdout/stderr separation, stable open JSON schemas, TTY/locale invariance, semantic help, error catalog; G-OUT and conformance tests |
| Destructive safety (R8) | conforming | cleanup target guards, explicit consent, `--no-input`, dry-run, invoking-cwd refusal; G-CLN |
| Publishable tier (R9) | conforming for pre-release product surface | completions, SemVer/deprecation policy, locale/signals, doctor, telemetry disclosure, CI conformance, clean wheel install |
