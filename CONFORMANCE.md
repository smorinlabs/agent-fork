...
| REQ-22 | Implemented | G-RBK failure, manual recovery, producer, SIGINT, and SIGTERM rows; T-RBK-08/T-RBK-09 extend the signal rows to the setup hook's process group, and T-CLI-53 asserts the 130/143 translation at the `main()` boundary the library-level rows never reached |
| REQ-23 | Implemented | G-VER full ladder, ignored-aware comparison, opt-out, rollback |
| REQ-24 | Implemented | G-INC include precedence and non-fatal setup hook; A12 adds provenance gating, process-group bounds, and disclosure — T-INC-08..16 (eligibility, policy, bounded escaped output, timeout reaping), T-CLI-51 (`--dry-run` disclosure), T-CLI-52 (`doctor` row and its failure condition), T-OUT-24 (JSON contract and stderr narration), T-CFG-24..26 (key resolution and A11-guard rejection) |
